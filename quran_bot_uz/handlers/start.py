...
# --- (Oldingi Namoz vaqtlari, Lokatsiya va Kun oyati kodlari o'zgarishsiz qoldi) ---
...
# =================================================================
# YANGI QISM: INLINE TUGMALARNI ESHITADIGAN MIYA (CALLBACK HANDLERS)
# =================================================================

# 1. Suralar ro'yxatini sahifalash (Oldingi / Keyingi)
...
async def surah_page_callback(call: CallbackQuery):
    ...
    await call.answer() # Yuklanish (soat) belgisini to'xtatadi

# 2. Bitta sura tanlanganda -> Uning oyatlari ro'yxatini ochish
...
# 3. Oyatlar ro'yxatini sahifalash (Baqara kabi uzun suralar uchun 30 tadan)
...
# 4. Orqaga (Suralar ro'yxatiga) qaytish tugmasi
...
# 5. Oyat tugmasi bosilganda -> Arabcha va o'zbekcha matnni + audio tugmalarni ko'rsatish
...
async def verse_clicked_callback(call: CallbackQuery):
    ...
    if verse:
        ...
        # Oyatni eski xabarni o'zgartirmasdan, yangi xabar qilib yuboramiz (qulayroq bo'lishi uchun)
        await call.message.answer(text, reply_markup=keyboard, parse_mode="Markdown")
    await call.answer()

# 6. Audio tugma bosilganda -> Ovozli mp3 jo'natish
...
async def audio_callback(call: CallbackQuery):
    ...
    if verse:
        # Qaysi qorini tanlaganiga qarab bazadan audioni ajratib olamiz
